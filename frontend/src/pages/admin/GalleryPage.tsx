...
import { downloadFile, saveFilesToLocalDirectory } from "@/utils/download";
...
const GalleryPage: React.FC = () => {
  ...
  const handleCreateFolder = async (name: string) => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Gagal membuat folder. Coba lagi.",
      );
      throw err;
    }
  };
  ...
  const handleDeletePhoto = async () => {
    ...
    try {
      ...
      const error = err as { response?: { data?: { message?: string } } };
      toast.error(
        error.response?.data?.message || "Gagal menghapus foto. Coba lagi.",
      );
    }
  };
  ...
  const handleMovePhoto = async (folderId: number | null) => {
    ...
    try {
      ...
      const error = err as { response?: { data?: { message?: string } } };
      toast.error(
        error.response?.data?.message || "Gagal memindahkan foto. Coba lagi.",
      );
    }
  };
  ...
  const handleBulkMove = async (folderId: number | null) => {
    if (selectedIds.size === 0) return;
    try {
      await bulkMovePhotos.mutateAsync({
  ...
  const handleBulkDownload = async () => {
    ...
    const filesToDownload = selected.map((p) => ({
      url: p.url,
      filename: p.filename || `PixelBooth-${p.id}.jpg`,
    }));

    // 1. Prioritaskan simpan langsung ke folder lokal via File System Access API (Chrome/Edge)
    if ("showDirectoryPicker" in window) {
      try {
        toast.info("Pilih folder penyimpanan lokal");
        const result = await saveFilesToLocalDirectory(
          filesToDownload,
          (current, total, filename) => {
            toast.info(`Menyimpan foto (${current}/${total}): ${filename}...`);
          },
        );
        toast.success(
          `${result.savedCount} foto berhasil disimpan ke folder "${result.dirName || "lokal"}"!`,
        );
        setIsBulkDownloading(false);
        return;
      } catch (err: any) {
        if (err?.name === "AbortError") {
          // User membatalkan pemilihan dialog folder
          toast.info("Pemilihan folder dibatalkan.");
          setIsBulkDownloading(false);
          return;
        }
        console.warn(
          "File System Access API fallback ke download standar:",
          err,
        );
      }
    }

    // 2. Fallback: Download standar browser berurutan
    toast.info(
      `Mengunduh ${selected.length} foto ke folder Unduhan browser...`,
    );
    try {
      ...
      toast.success(`${selected.length} foto berhasil diunduh.`);
    } catch {
  ...
  const handlePrintAllInFolder = () => {
    ...
    const currentName =
      breadcrumb.length > 0 ? breadcrumb[breadcrumb.length - 1].name : "Galeri";
    setPrintModalState({
      isOpen: true,
      photos: photos.map((p) => ({
        id: p.id,
  ...
  const handlePrintFolderCard = async (folder: Folder) => {
    try {
      ...
      if (folderPhotos.length === 0) {
        toast.info(
          `Folder "${folder.name}" belum memiliki foto untuk dicetak.`,
        );
        return;
      }
      setPrintModalState({
  ...
  return (
    <div className="flex flex-col w-full pb-12">
          ...
          <div className="flex items-center gap-2 font-retro text-sm sm:text-base text-[var(--pb-text-muted)] mb-1 tracking-wide flex-wrap">
            ...
            {breadcrumb.map((crumb, index) => (
              <React.Fragment key={crumb.id}>
                <span className="text-[#FF5A36] font-pixel text-[9px]">
                  &gt;
                </span>
                <button
                  type="button"
          ...
          {photos.length > 0 && (
            <Button
              ...
              leftIcon={
                <Printer
                  size={15}
                  className="text-[#FFB800] stroke-[2.5] shrink-0"
                />
              }
      ...
      <div className="w-full">
              ...
              {folderSelectionMode ? (
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 p-3 rounded-[4px] bg-[var(--pb-surface)] border-[2px] border-[var(--pb-border-strong)] shadow-[3px_3px_0px_var(--pb-shadow-solid)] w-full">
                  ...
                  <div className="flex items-center justify-between sm:justify-start gap-2.5">
                    ...
                    >
                      {allFoldersSelected ? (
                        <CheckSquare size={14} className="text-[#FF5A36]" />
                      ) : (
                        <Square size={14} />
                      )}
                      <span>
                        {allFoldersSelected ? "Batal Semua" : "Pilih Semua"}
                      </span>
                    </button>
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      leftIcon={
                        <FolderInput
                          size={14}
                          className="text-[var(--pb-yellow)] stroke-[2.5]"
                        />
                      }
                    >
                      Pindahkan
          ...
          >
            <div className="bg-pb-surface border border-pb-border rounded-2xl">
              <EmptyState
                ...
                title={
                  activeFolderId ? "Belum ada sub-folder" : "Belum ada folder"
                }
                description={
                  activeFolderId
        ...
        <div className={activeFolderId ? "" : "mt-8"}>
          ...
          <PhotoGrid
            ...
            isBulkActionPending={
              bulkDeletePhotos.isPending || bulkMovePhotos.isPending
            }
      ...
      <MovePhotoModal
        ...
        excludeFolderIds={
          moveTarget?.folder_id != null ? [moveTarget.folder_id] : []
        }
      />
      ...
      <MovePhotoModal
        ...
        excludeFolderIds={(() => {
          ...
          return first != null && sel.every((p) => p.folder_id === first)
            ? [first]
            : [];
        })()}
      />
